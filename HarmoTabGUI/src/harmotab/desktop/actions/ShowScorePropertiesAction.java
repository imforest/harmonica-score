...
public class ShowScorePropertiesAction extends UserAction {
	...
	public ShowScorePropertiesAction() {
		super(
			Localizer.get(i18n.MENU_SCORE_PROPERTIES), 
			ActionIcon.getIcon(ActionIcon.SCORE_PROPERTIES)
		);
		setLittleIcon(ActionIcon.getIcon(ActionIcon.SCORE_PROPERTIES_LITTLE));
	}

	@Override
	public void run() {
		ScoreController controller = DesktopController.getInstance().getScoreController();
		ScoreSetupDialog setupDlg = new ScoreSetupDialog(null, controller);
