...
/**
 * Enregistrement d'une interpr�tation � partir d'une partition ht3
 */
public class RecordPerformanceFromHt3Action extends UserAction {
	...
	public RecordPerformanceFromHt3Action() {
		super(
			Localizer.get(i18n.MENU_RECORD),
			ActionIcon.getIcon(ActionIcon.RECORD)
		);
	}
	...
	public void run() {
		...
		int res = JOptionPane.showConfirmDialog(
			null, 
			Localizer.get(i18n.M_RECORD_FROM_SCORE_CONFIRMATION),
			Localizer.get(i18n.N_RECORDING),
			JOptionPane.YES_NO_OPTION);
		
		if (res == JOptionPane.YES_OPTION) {
			if (controller.saveScore()) {
				File exportedFile = controller.exportAsExportedScore();
