...
public class InsertAfterAction extends UserAction {
	...
	public InsertAfterAction() {
		super(
			Localizer.get(i18n.MENU_INSERT_AFTER),
			ActionIcon.getIcon(ActionIcon.ADD_AFTER)
		);
		setLittleIcon(ActionIcon.getIcon(ActionIcon.ADD_AFTER_LITTLE));
	}
	...
	public void run() {
		...
		if (selection != null) {
			...
			AddElementMenu menu = AddElementMenu.createInsertAfter(track, element);
			menu.populate();
